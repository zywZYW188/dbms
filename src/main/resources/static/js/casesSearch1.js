...
function initEchart(){
    ...
    //$(".kpiChartContainer").hide(); //hide是将.kpiChartContainer框隐藏了
}
...
function drawCanvas(canvas) {
    ...
    if (canvas.getContext) {
        ...
        context.fillRect(initSetx, initSety, width, height - 100);//画布填充范围
        var d = 5;
        var ordinateN = width / d;//大格子的竖线个数
        var abscissaN = (height - 100) / d;//大格子的横线个数
        var startPoint = {x: 50, y: 50};//起点坐标
        ...
        for (var i = 0; i <= ordinateN; i++) {
            ...
            context.lineTo(startPoint.x, startPoint.y + height - 100);
            startPoint.x += d;
            context.stroke();
        }
        //绘制横线
...
function fillMsg(domClassName, ecgLead, data) {
    ...
    ctx.strokeText(ecgLead, data[0].x + 50, data[0].y - 20);//目前的大小只能适应平板
    var basicMsg = " 横向：0.04S/格 纵向：0.1mV/格";
    var textPosition1 = (canvas.width - ctx.measureText(basicMsg).width);
    ctx.fillText(basicMsg, textPosition1, 30);
}
